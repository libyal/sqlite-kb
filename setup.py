...
# Change PYTHONPATH to include sqliterc so that we can get the version.
sys.path.insert(0, '.')
    ...
    def _make_spec_file(self):
      ...
      for line in iter(spec_file):
        if line.startswith('Summary: '):
          ...
          lines.extend([
              '%{python3_sitelib}/sqliterc/*.py',
              '%{python3_sitelib}/sqliterc*.egg-info/*',
              '',
              '%exclude %{_prefix}/share/doc/*',
              '%exclude %{python3_sitelib}/sqliterc/__pycache__/*',
              '%exclude %{_bindir}/*.py'])
          ...
          python_spec_file.extend([
              ...
              f'Requires: {python_package:s}-sqlite-kb >= %{{version}}',
...
sqliterc_description = (
    'SQLite database resources (sqliterc)')

sqliterc_long_description = (
    'sqliterc is a Python module part of sqlite-kb to allow reuse of SQLite '
    'database resources.')
...
setup(
    ...
    description=sqliterc_description,
    long_description=sqliterc_long_description,
